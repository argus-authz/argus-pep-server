...
public class DFPMObligationHandlerConfigurationParser
  ...
  /**
   * The name of the {@value} property which gives ID of the obligation handled
   * by this obligation handler
   */
  public static final String HANDLED_OBLIGATION_ID_PROP = "handledObligationId";

  /**
   * The default value of the {@value #HANDLED_OBLIGATION_ID_PROP} property:
   * {@value} .
   */
   ...
   * associated with the subject's DN should be preferred over the one from the
   * primary FQAN.
   */
  ...
  /**
   * The name of the {@value} property that indicates that the failure to find a
   * primary group mapping in the group map file cause the obligation handler to
   * fail.
   */
  ...
  /**
   * The name of the {@value} property which gives the the absolute path to the
   * directory that grid mappings are stored.
   */
  public static final String GRID_MAP_DIR_PROP = "gridMapDir";

  /**
   * The name of {@value} property which gives the maximum number of iterations
   * allowed for a pool account lookup.
   */
  public static final String POOL_ACCOUNT_LOOKOUP_MAX_ITERATIONS_PROP = "poolAccountLookupMaxIterations";

  /**
   * The name of {@value} property which enables the use of the caching pool
   * account resolver.
   */
  public static final String ENABLE_POOL_ACCOUNT_CACHE_PROP = "enablePoolAccountCache";

  /**
   * The name of {@value} property which gives the TTL for the pool account
   * cache, in minutes.
   */
  public static final String POOL_ACCOUNT_CACHE_TTL_PROP = "poolAccountCacheTTL";

  /**
   * The default value of the {@value #ENABLE_POOL_ACCOUNT_CACHE_PROP} property:
   * {@value}
   */
  public static final boolean ENABLE_POOL_ACCOUNT_CACHE = true;

  /**
   * The default value of the {@value #POOL_ACCOUNT_CACHE_TTL_PROP} property:
   * {@value}
   */
  public static final int POOL_ACCOUNT_CACHE_TTL = 5;

  /**
   * The default valuye of the {@valu #POOL_ACCOUNT_LOOKOUP_MAX_ITERATIONS_PROP}
   * property: {@value}
   */
  public static final int POOL_ACCOUNT_MAX_ITERATIONS = 10;

  /**
   * The default value of the {@value #PREFER_DN_FOR_LOGIN_NAME_PROP} property:
   * {@value} .
   */
  ...
  private final Logger log = LoggerFactory
    .getLogger(DFPMObligationHandlerConfigurationParser.class);
  ...
  public ObligationHandler parse(Ini.Section iniConfig,
    ...
    String obligationId = IniConfigUtil.getString(iniConfig,
      HANDLED_OBLIGATION_ID_PROP, HANDLED_OBLIGATION_ID_DEFAULT);
    ...
    int mapRefreshPeriod = IniConfigUtil.getInt(iniConfig,
      MAP_REFRESH_PERIOD_PROP, MAP_REFRESH_PERIOD_DEFAULT, 1,
      Integer.MAX_VALUE);
    log.info("{}: mapping file(s) refresh period: {} mins", name,
      mapRefreshPeriod);

    /* mapping options: DN have precedence over FQAN ? */
    boolean preferDNForLoginName = IniConfigUtil.getBoolean(iniConfig,
      PREFER_DN_FOR_LOGIN_NAME_PROP, PREFER_DN_FOR_LOGIN_NAME_DEFAULT);
    log.info("{}: prefer DN login name mappings: {}", name,
    ...
    log.info("{}: prefer DN primary group mappings: {}", name,
    ...
    log.info("{}: no primary group name mapping is error: {}", name,
      noPrimaryGroupNameIsError);

    boolean useSecondaryGroupNamesForMapping = IniConfigUtil.getBoolean(
      iniConfig, USE_SECONDARY_GROUP_NAMES_FOR_MAPPING_PROP,
      USE_SECONDARY_GROUP_NAMES_FOR_MAPPING_DEFAULT);
    log.info("{}: use secondary group names for mapping (lease filename): {}",
      name, useSecondaryGroupNamesForMapping);

    boolean enablePoolAccountCache = IniConfigUtil.getBoolean(iniConfig,
      ENABLE_POOL_ACCOUNT_CACHE_PROP, ENABLE_POOL_ACCOUNT_CACHE);

    log.info("{}: enable pool account cache: {}", name, enablePoolAccountCache);

    int poolAccountCacheTTL = IniConfigUtil.getInt(iniConfig,
      POOL_ACCOUNT_CACHE_TTL_PROP, POOL_ACCOUNT_CACHE_TTL, 0,
      Integer.MAX_VALUE);

    log.info("{}: pool account cache ttl (in minutes): {}", name,
      poolAccountCacheTTL);

    int poolAccountMaxIterations = IniConfigUtil.getInt(iniConfig,
      POOL_ACCOUNT_LOOKOUP_MAX_ITERATIONS_PROP, POOL_ACCOUNT_MAX_ITERATIONS, 1,
      Integer.MAX_VALUE);

    log.info("{]: pool account lookup max iterations: {}", name,
      poolAccountMaxIterations);

    GridmapDirGetMappingStrategy mappingStrategy = buildMappingStrategy(
      gridMapDir, poolAccountMaxIterations, enablePoolAccountCache,
      poolAccountCacheTTL);

    PoolAccountManager poolAccountManager = buildPoolAccountManager(gridMapDir,
      useSecondaryGroupNamesForMapping, mappingStrategy);

    AccountMapper accountMapper = buildAccountMapper(accountMapFile,
      preferDNForLoginName, groupMapFile, preferDNForPrimaryGroupName,
      mapRefreshPeriod * 60 * 1000, noPrimaryGroupNameIsError,
      poolAccountManager);

    DFPMObligationHandler obligationHandler = new DFPMObligationHandler(name,
      obligationId, accountMapper);
    ...
    boolean requireSubjectKeyInfo = IniConfigUtil.getBoolean(iniConfig,
      REQUIRE_SUBJECT_KEYINFO_PROP, REQUIRE_SUBJECT_KEYINFO_DEFAULT);
    log.info("{}: requires subject key-info attribute to apply: {}", name,
   ...
   *          whether primary group derived from DN mappings should be preferred
   ...
   *          whether the failure to map a primary group name cause an error or
   *          not
   * 
   * @return the constructed account mapper
   * 
   * @throws ConfigurationException
   *           thrown if the mapping files can not be read or parsed or if the
   *           grid map directory is not read and writable
   */
  private AccountMapper buildAccountMapper(String accountMapFile,
    ...
    boolean noPrimaryGroupNameIsError, PoolAccountManager poolAccountManager)
      throws ConfigurationException {
    ...
    AccountIndicatorMappingStrategy aimStrategy = new DNPrimaryFQANAccountIndicatorMappingStrategy(
      accountIndicatorMap, dnMatchStrategy, fqanMatchStrategy,
      preferDNMappingForAccountIndicator);

    // group names mapping
    GroupNameMappingStrategy gnmStrategy = new DNFQANGroupNameMappingStrategy(
      groupMap, dnMatchStrategy, fqanMatchStrategy,
      preferDNMappingForPrimaryGroupName);

    return new AccountMapper(aimStrategy, gnmStrategy, poolAccountManager,
      noPrimaryGroupNameIsError);
  }
   ...
   * @param poolAccountLookupMaxIterations
   * @param enablePoolAccountCache
   * @param poolAccountCacheTTL
   * @return
   */
  private GridmapDirGetMappingStrategy buildMappingStrategy(
    String gridMapDirPath, int poolAccountLookupMaxIterations,
    boolean enablePoolAccountCache, long poolAccountCacheTTL) {

    PoolAccountResolver resolver;

    File gridMapDir = new File(gridMapDirPath);

    if (enablePoolAccountCache) {
      resolver = new CachingPoolAccountResolver(gridMapDir, poolAccountCacheTTL,
        TimeUnit.MINUTES);
    } else {
      resolver = new DefaultPoolAccountResolver(gridMapDir);
    }

    GridmapDirGetMappingStrategy mappingStrategy = LockFreeMappingStrategy
      .forGridmapDir(gridMapDir)
      .withMaxLookupIterations(poolAccountLookupMaxIterations)
      .withPoolAccountResolver(resolver).withShuffleAccounts(true).build();
   ...
   * 
   * @throws ConfigurationException
   *           thrown if the given grid map directory is not a directory, can
   *           not be read, or can not be written to
   */
  private PoolAccountManager buildPoolAccountManager(String gridMapDirPath,
    boolean useSecondaryGroupNamesForMapping,
    GridmapDirGetMappingStrategy mappingStrategy)
      throws ConfigurationException {
    ...
    if (!gridMapDir.exists()) {
      ...
      String errMsg = MessageFormatter
        .format("Grid map directory {} does not exist",
    ...
    if (!gridMapDir.canRead()) {
      String errMsg = MessageFormatter
        .format("Grid map directory {} is not readable by this process",
          gridMapDir.getAbsolutePath())
        .getMessage();
      log.error(errMsg);
      throw new ConfigurationException(errMsg);
    ...
    if (!gridMapDir.canWrite()) {
      String errMsg = MessageFormatter
        .format("Grid map directory {} is not writable by this process",
          gridMapDir.getAbsolutePath())
        .getMessage();
      log.error(errMsg);
      throw new ConfigurationException(errMsg);
    ...
    GridMapDirPoolAccountManager poolAccountManager = new GridMapDirPoolAccountManager(
      mappingStrategy, gridMapDir, useSecondaryGroupNamesForMapping);
