...
public class OidcProfilePIPConfigurationParser
  ...
  private static final Logger LOG = LoggerFactory
    .getLogger(OidcProfilePIPConfigurationParser.class);
  ...
  public PolicyInformationPoint parse(Section iniConfig,
    ...
    String oidcClientUrl = IniConfigUtil.getString(iniConfig,
      OIDC_CLIENT_URL_PROP, DEFAULT_OIDC_URL);
    LOG.info(LOG_PROP_PATTERN,
      new Object[] { pipId, OIDC_CLIENT_URL_PROP, oidcClientUrl });

    Integer oidcCacheTTL = IniConfigUtil.getInt(iniConfig, OIDC_CACHE_TTL_PROP,
      DEFAULT_OIDC_CACHE_TTL, 0, 3600);
    LOG.info(LOG_PROP_PATTERN,
      new Object[] { pipId, OIDC_CACHE_TTL_PROP, oidcCacheTTL });

    Integer oidcCacheMaxElements = IniConfigUtil.getInt(iniConfig,
      OIDC_CACHE_MAX_ELEMS_PROP, DEFAULT_OIDC_CACHE_MAX_ELEMENTS, 0,
      Integer.MAX_VALUE);
    LOG.info(LOG_PROP_PATTERN,
      new Object[] { pipId, OIDC_CACHE_MAX_ELEMS_PROP, oidcCacheMaxElements });

    Boolean oidcCacheEnabled = IniConfigUtil.getBoolean(iniConfig,
      OIDC_CACHE_DISABLED_PROP, DEFAULT_OIDC_CACHE_DISABLED);
    LOG.info(LOG_PROP_PATTERN,
      new Object[] { pipId, OIDC_CACHE_DISABLED_PROP, oidcCacheEnabled });

    try {
      OidcProfileToken tokenService = new OidcProfileTokenImpl();
      TokenDecoder decoder = new TokenDecoderImpl(oidcClientUrl, oidcCacheTTL,
        oidcCacheMaxElements, oidcCacheEnabled);
      return new OidcProfilePIP(pipId, tokenService, decoder);

    } catch (Exception e) {
      String errorMsg = String.format("%s: error building OIDC profile PIP: %s",
        pipId, e.getMessage());
