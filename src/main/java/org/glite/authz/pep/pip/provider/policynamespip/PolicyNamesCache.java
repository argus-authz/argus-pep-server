...
public class PolicyNamesCache {
  ...
  private static final Logger LOG = LoggerFactory
    .getLogger(PolicyNamesCache.class);
   ...
   * @throws IOException
   *           on read errors for trustDir or one of the info files
  ...
  /**
   * Tries to find given issuer DN in the cached info files in the
   * {@link #trustDir}.
   * 
   * @param issuerDN
   *          String issuer DN to look for
   ...
   * 
   * @param oldCache
   *          previous PolicyNamesCache to be updated
   * @see #update()
   * @throws IOException
   *           upon reading errors
   */
  private void update(PolicyNamesCache oldCache) throws IOException {
    ...
    // Log statistics
    LOG.debug(
      "Updated list ({}): {} msec ({} info files, {} valid, {} external dep(s), {} copied, {} failed, {} new)",
      trustDir, (System.nanoTime() - t0) / 1000000.0, numInfoFiles,
      infoEntries.size(), extInfoEntries.size(), numEntriesCopied,
      numEntriesFailed, numEntriesNew);
  }
   ...
   * 
   * @param trustDir
   *          directory containing {@value #FILE_SFX} files
   * @return ArrayList of Path
   * @throws IOException
   *           upon directory reading errors
   */
   ...
   * where needed). The entry is first searched for in the oldList and used when
   * unchanged. Otherwise a new entry is parsed. Following that, the list of
   * 'external' dependencies (i.e. those outside the {@link #trustDir} or not
   * ending with {@value #FILE_SFX}) is handled, by recursively calling
   * ourselves.
   * 
   * @param path
   *          path of entry to handle
   * @param oldList
   *          either {@link #oldInfoEntries} or {@link #oldExtInfoEntries}
   *          depending on the type of entry
   * @param newList
   *          either {@link #infoEntries} or {@link #extInfoEntries} depending
   *          on the type of entry
   * @param recursion
   *          level of recursion (max. {@value #MAX_RECURSION})
   * @throws IOException
   *           when failing to read a info file
   * @throws ParseException
   *           on too many levels of recursion
   */
  private void handleEntry(Path path, LinkedHashMap<Path, Entry> oldList,
    LinkedHashMap<Path, Entry> newList, int recursion)
    throws IOException, ParseException {

    // Protect against recursion
    if (recursion > MAX_RECURSION) {
      throw new ParseException(
        format("Too many levels of recursion (max. {}) in {}", MAX_RECURSION,
          path),
        recursion);
    }
    ...
    if (Files.notExists(path)) {
      LOG.warn("Skipping non-existing {}", path.getFileName()
        .toString());
      numEntriesFailed++;
      return;
    }
    ...
    if (Files.isSymbolicLink(path)) {
      ...
      FileTime mod_link = Files.getLastModifiedTime(path,
    ...
    if (entry == null || !entry.modified.equals(modified)) {
      ...
      try {
        ...
        LOG.warn("Syntax error, skipping {}", path.getFileName()
          .toString());
        numEntriesFailed++;
        return;
      }
   ...
   * @throws ParseException
   *           in case of subjectdn parsing errors
   */
  private Entry parseInfoFile(Path path, FileTime modified)
    ...
    if (value == null || value.isEmpty()) {
      throw new ParseException(
        format("%s: No or empty %s key found", path.getFileName(), SUBJECT_KEY),
        0);
    }
  ...
  private String getSubjectDNvalue(Path path) throws IOException {
    ...
    try {
      ...
      final String errorMsg = format("Cannot open %s: %s", name,
        e.getMessage());
      LOG.error(errorMsg);
    ...
    try {
      while ((newline = reader.readLine()) != null) {
        ...
        if (!line.isEmpty() && line.charAt(0) != '#') {
          ...
          if (sep >= 0 && SUBJECT_KEY.equals(line.substring(0, sep)
            .trim())) {
      ...
      // one in any case.
      final String errorMsg = format("Reading from %s failed: %s", name,
        e.getMessage());
      LOG.error(errorMsg);
      try {
        reader.close();
   ...
   *          {@link Entry} to fill
   * @param value
   *          value of the subjectdn key
   * @throws ParseException
   *           on syntax errors in the value
   * @throws IOException
   *           on read errors with one of the info files
   */
  private void parseSubjectDnValue(Entry entry, String value)
    ...
    while (true) {
      ...
      if (value.charAt(pos) == '"') {
        ...
        if (pos2 < 0) {
          throw new ParseException(format("%s: Missing end-quote", entry.name),
            pos + 1);
        }
        // Add url-decoded value to the subDN list
        entry.localSubDNs
          .add(URLDecoder.decode(value.substring(pos + 1, pos2), "UTF-8"));
        // skip over end-quote
        pos2++;
      } else if (value.substring(pos)
        .startsWith("file:")) {
        // Found file: look for end of filename
        for (pos2 = pos + 5; pos2 < value.length() && value.charAt(pos2) != ','
          && value.charAt(pos2) != ' ' && value.charAt(pos2) != '\t'; pos2++)
          ;
        ...
        throw new ParseException(
          format("%s: %s value invalid: %s", entry.name, SUBJECT_KEY, value),
          pos);
      }
      ...
      if (!foundComma) {
        throw new ParseException(
          format("%s: Missing comma delimiter before new entry", entry.name),
          pos);
      }
    }
  }
  ...
  private class Entry {
    ...
    private Entry(Path path, FileTime modified) {
      ...
      this.name = (name.endsWith(FILE_SFX)
        ? name.substring(0, name.length() - FILE_SFX.length()) : name);
      this.path = path;
     ...
     * dependency list. A path outside the trustDir or a path not ending with
     * {@link #FILE_SFX} is external.
     * 
     * @param dependency
     *          Path of the dependency to find
     */
    private void addDependency(String dependency) {
      ...
      if (trustDir.equals(depPath.getParent()
        .toString())
        && depPath.getFileName()
          .toString()
          .endsWith(FILE_SFX)) {
     ...
     * 
     * @param recursion
     *          level of recursion (max. {@link #MAX_RECURSION})
    ...
    private HashSet<String> getSubDNs(int recursion) throws ParseException {
      ...
      if (recursion > MAX_RECURSION) {
        throw new ParseException(
          format("Too many levels of recursion (max. %d) in %s", MAX_RECURSION,
            name),
          recursion);
      }
    ...
    /**
     * Recursively updates the internal set of all subject DNs valid for this
     * entry.
     */
