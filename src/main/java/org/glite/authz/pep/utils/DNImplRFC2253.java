 ...
 * DNImpl.java
 * 
 * @author Joni Hahkala, Created on September 8, 2003, 7:21 PM
...
public class DNImplRFC2253 implements DN {
    ...
    /** Logging facility. */
    private final Logger LOGGER= LoggerFactory.getLogger(DNImplRFC2253.class);
     ...
     */
    public DNImplRFC2253(DERObjectIdentifier[] newOids, String[] newRdns,
                         int newCount) {
        m_oids= newOids;
        m_rdns= newRdns;
        m_count= newCount;
    ...
    /**
     * Creates a new DNImpl object. Assumes the string represents either an old
     * openssl x500 format DN in direct format or a proper reversed RFC2253
     * format DN.
     * 
     * @param name
     ...
     *            Generates a new DNImpl class from the DN in the name.
     * @param reversed
     *            Whether the given DN is reversed or not for proper RFC 2254
     *            DNs this should be true.
     */
    ...
    public DNImplRFC2253(Principal principal) {
        ...
        // LOGGER.debug("input is: " + principal.getClass().getName() +
        // " from classloader: " + principal.getClass().getClassLoader() +
        // " current one is: " + getClass().getClassLoader());
        ...
        if (principal instanceof X509Principal) {
            ...
            if (principal instanceof X500Principal) {
                // for X500Principal, get the encoded and reparse as
                // bouncycastle X509Principal.
                ...
                // UGLY HACK, shouldn't do this, but there seems to be no way
                // around it, input can be many classes that give the DN in
                // different orders. And from different classloaders preventing
                // casts etc.
                // if DN starts with email or CN, it's in reversed order
                // LOGGER.debug("test name: " + testName);
                if (testName.startsWith("email") || testName.startsWith("e=")
                        || testName.startsWith("cn=")
                        || testName.startsWith("uid=")
                        || testName.startsWith("sn=")) {
                    ...
                    if (testName.startsWith("c=") || testName.startsWith("st=")
                            || testName.startsWith("ou=")
                            || testName.startsWith("dc=")
                            || testName.startsWith("o=")) {
                        x509Principal= new X509Principal(false, principal.getName());
                        // LOGGER.debug("country first, reverse " +
                        // x509Principal);
                    }
                    else {
                        // check if it end with CN, email, UID or SN, and then
                        // not flip it.
                        x509Principal= new X509Principal(false, principal.getName());
                        Vector oids= x509Principal.getOIDs();
                        String rdn= ((DERObjectIdentifier) oids.lastElement()).getId();
                        if (rdn.equals(X509Name.CN.getId())
                                || rdn.equals(X509Name.E.getId())
                                || rdn.equals(X509Name.UID.getId())
                                || rdn.equals(X509Name.SN.getId())) {
                            x509Principal= new X509Principal(false, principal.getName());
                        }
                        else {
     ...
     * 
     * @param canon
     *            Whether the canonical (lowercase) format is wanted.
     * @param reverse
     *            Whether to reverse the DN (yes for actual rfc 2253
     *            conformance, CN first).
     * @param lookup
     *            The table to lookup the RDN description strings (CN, O, C
     *            etc.).
     * @return The constructed string.
     */
    private String constructRFC2253(boolean canon, boolean reverse,
                                    Hashtable lookup) {
        StringBuffer buf= new StringBuffer();
        boolean first= true;
     ...
     * 
     * @param buf
     *            The buffer where the DN is being constructed.
     * @param n
     *            The index of the RDN to add.
     * @param first
     *            Whether this is the first RDN.
     * @param lookup
     *            There to lookup the textual representations of the RDN type.
     */
     ...
     * @deprecated internal method, replaced with parse(String inputDN, boolean
     *             reversed)
     */
     ...
     *            the string that contains the DN to parse.
     * @param reversed
     *            Whether the given DN is to be considered reversed or not.
     */
     ...
     * @param inputDN
     *            the string that contains the DN to parse.
     * @deprecated internal method, replaced with parseX500(String inputDN,
     *             boolean reversed)
     */
     ...
     * case the DN doesn't end with CN or in case the proxy checking is used and
    ...
    public DN withoutLastCN(boolean checkProxy) {
        ...
        if (checkProxy) {
            if (!m_rdns[m_count - 1].matches("^((limited )?proxy|[0-9]*)$")) {
                throw new IllegalArgumentException("Trying to remove the last proxy CN from DN that does not end in proxy CN. DN was: "
                        + toString());
            }
        }
     ...
     */
    public int hashCode() {
        return java.util.Arrays.hashCode(m_rdns)
                + java.util.Arrays.hashCode(m_oids) + m_count;
    }
