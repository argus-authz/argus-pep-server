...
public class X509ExtractorPIPTest {
  ...
  /** Invalid PEM string */
  private final String PEM_invalid = "-----BEGIN CERTIFICATE-----\n" + "AAP\n"
    + "-----END CERTIFICATE-----";

  /** Valid certificate with OIDs */
  private final String PEM_OIDs = "-----BEGIN CERTIFICATE-----\n"
  ...
  private final String PEM_Basic = "-----BEGIN CERTIFICATE-----\n"
  ...
  private final String PEM_EmptyIssuer = "-----BEGIN CERTIFICATE-----\n"
  ...
  private final String Base64Der_Basic1 = "MIIBuTCCAWOgAwIBAgIENl1jpTANBgkqhkiG9w0BAQsFADBAMQswCQYDVQQGEwJO\n"
  ...
  private final String Base64Der_Basic2 = "MIIB0DCCAXqgAwIBAgIBATANBgkqhkiG9w0BAQsFADA/MQswCQYDVQQGEwJOTDEQ\n"
    + "MA4GA1UECgwHRXhhbXBsZTEMMAoGA1UECwwDUERQMRAwDgYDVQQDDAdUZXN0IENB\n"
  ...
  /**
   * Setup a new {@link X509ExtractorPIP} PIP and set the accepted attributes to
   * all of them
   */
  @Before
  public void initialize() throws Exception {
    ...
    pip.setAcceptedAttrIDs(
      new AcceptedAttr[] { AcceptedAttr.ACCEPT_ATTR_X509_ISSUER,
   ...
   *          input request
   * @param element
   *          String which should match the stacktrace message
   * @return boolean: true when PIP has applied, false otherwise
   */
  protected boolean throwsPIPProcessingException(Request request,
    String element) {
  ...
  public void testValidRequestEmptyIssuer() throws Exception {
    ...
    boolean result = pip.populateRequest(request);
    Attribute[] attr = request.getSubjects()
      .toArray(new Subject[0])[0].getAttributes()
        .toArray(new Attribute[0]);
    assertTrue("populateRequest should have succeeded", result);
    assertTrue("subject should have 3 attributes, found " + attr.length,
      attr.length == 3);
  }
  ...
  public void testValidRequestBasic() throws Exception {
    ...
    Attribute[] attr = request.getSubjects()
      .toArray(new Subject[0])[0].getAttributes()
        .toArray(new Attribute[0]);
    assertTrue("populateRequest should have succeeded", result);
    assertTrue("subject should have 2 attributes, found " + attr.length,
      attr.length == 2);
  }
